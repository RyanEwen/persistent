...
> npm run verify:android     # re-sync android-plugin/ + ./gradlew :app:compileDebugKotlin
> ```
>
> Use `verify:android` as the quick check after editing the Kotlin in
> `android-plugin/` — it re-copies the sources and compiles them (no full APK).
