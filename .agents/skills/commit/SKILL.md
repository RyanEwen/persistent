...
Invocation input (optional): the user-provided message hint, summary, push
request, or pause request.
...
- `README.md` (root): stack, features, auth, sync model, development,
  deployment, releases, and the Codex skill list.
- `apps/mobile/README.md`: Android build, run, signing, update, and native plugin flow.
- Every tracked `AGENTS.md` plus `.agents/guides/`: conventions, helper
  inventories, and invariants affected by the change. Enumerate scoped files
  with `git ls-files '*AGENTS.md'` rather than maintaining a duplicate list.
...
- `.agents/skills/`: when a workflow, script, or expectation changed.
- `.env.example`: when any environment variable or config key changed.
- Build/deploy descriptors: `Dockerfile`, `compose.server.yml`,
  `.github/workflows/*`, and package scripts. Keep referenced
...
Data-isolation review scope:
- Read `apps/api/AGENTS.md` and `docs/auth-architecture.md` for the full model
  exceptions. Verify every changed domain query uses the authenticated user's
  scope, and that edit/delete paths re-fetch ownership rather than trusting a
  path id.
- Validate new request bodies at the boundary with a Zod schema from
  `@persistent/shared`.
...
Notes:
- `npm run validate` needs neither a database nor `DATABASE_URL`; the Prisma
  validation script supplies a non-connecting placeholder when unset.
