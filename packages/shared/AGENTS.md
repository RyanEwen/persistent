# Shared package guidance (`packages/shared`)
...
- `reminderTypes` is every stored reminder type; `selectableReminderTypes` is
  the editor's list. Medication is available for new reminders.
